...
- **Origin:** The autoresearcher was originally written inside GlucoAssist at `backend/app/services/autoresearcher.py` (last vendored commit: `9c3de77`) and extracted to this standalone container. It is a **domain adaptation of [karpathy/autoresearch](https://github.com/karpathy/autoresearch)** — not a git fork, but a direct conceptual port. Karpathy's version has an LLM autonomously modify a PyTorch `train.py` to minimize GPT validation loss; ours has the LLM propose JSON feature/model configs evaluated via walk-forward CV on glucose data. The structural mapping is: `program.md` → `autoresearcher_program.md`, `results.tsv` → `autoresearcher_log` table, `val_bpb` metric → MAE at 30/60/90/120 min horizons, promoted commits → `promoted_model_config.json`. **Sync strategy:** no git sync. The only upstream file worth monitoring is `program.md` — when Karpathy improves the research loop instructions (experiment discipline, LLM guidance patterns, loop structure), port relevant improvements to `app/services/autoresearcher_program.md`.
